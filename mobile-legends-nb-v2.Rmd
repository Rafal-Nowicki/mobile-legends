...
Another issue is the choice of data points we are going to calculate the distance between. Also in this case there are several possibilities. The most-used ones are single linkage, complete linkage, average linkage and Ward method. In single linkage approach we connenct the to-be-connected sets based on the datapoints that are closest to each other. Complete linkage works in the opposite way - we connect the datapoints based on the maximal distance between the sets. Average method is a compromise between these two approaches (see the picture). Ward's method is a bit different than the previous ones. Using that method we create clusters for which the variance witihin the groups is minimized.

In our analysis we will use the agglomerative algorithm with euclidean metrics and Ward's method of linking.

One huge advantage of hierarchical clustering methods over for instance k-means clustering algorithm is that we can get a valuable insight of the structure of our data by looking on the so called dendrogram. Domain knowledge is very helpful at that point as it might be way easier to work out the number of clusters and their possible names.
...
dend <- scores %>% dist("euclidean") %>% 
   hclust(method = "ward.D2") %>% as.dendrogram %>%  set("branches_k_color", k=5) %>% set("branches_lwd", 1.2) %>%
...
Although the choice of the threshold, (i.e. how many cluster we want to distinguish) is arbitrary we can look at some metrics that can reflect something similar to goodness of fit in clustering framework. For instance we can compute a so called silhoutte width for every observation to see how similar that observation is with the cluster it was assigned to. If we average out all the computed silhoutte widths for different number of cluster and plot it we might get nice insight on what is going on in the data structure.
...
Below you can see four versions of graph discussed above. The first one and the third one suggest there are seven clusters in our sample. The fourth one is quite close to them as it indicates there are eight clusters. Silhoutte plot for single linkage method suggests we should distinc only two clusters. Game knowledge might be very helpful now so could understand that matter better.

Let see how does it look like if we choose 2 clusters.

```{r, warning=F, message = F, echo = F}
CLUSTER <- as.factor(cutree(dend, 2))

df_class <- cbind(df_num,CLUSTER)

ggplot <- function(...) ggplot2::ggplot(...) +scale_color_manual(values = c("steelblue", "darkred")) + scale_fill_manual(values = c("steelblue", "darkred"))

unlockBinding("ggplot",parent.env(asNamespace("GGally")))
assign("ggplot",ggplot,parent.env(asNamespace("GGally")))

s <- ggpairs(data=df_class, columns=2:9, mapping = aes(colour = CLUSTER),
...
df_class %>% 
  ...
  ggradar(base.size = 1, plot.legend = FALSE) +
...
Another way to present dendrogram, however not that insightful, is very fancy so I will just drop it here since it is very nice plot to show :)
