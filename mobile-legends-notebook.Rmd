...
var.labels = c(HERO="Hero", MV_SPD="Movement speed",
...
summarize_if(is.numeric, cv) %>%
  kable(col.names = label(df)[-c(1,11,12)]) %>%
...
rownames(df) <- df$HERO
pca <- prcomp(df[-c(1,10,11)], scale = TRUE)
```
...
rownames(eig.tab) <- paste0("PCA",1:8)
...
As you can see in the table above they account for about 73,5% of data variability - that's fine. However in order to make interpretations of the components easier we are going to incorporate 4 of them in our further analysis. In that way almost 83% of the variance will be explained.
...
Now as we reduced dimentionality we can proceed to the most exciting part of our analysis - clusters distinguishment. To do so we will implement k-means algorithm.
